module Restful
  class Preparer
    def initialize options
      @resource = options[:resource] || fail("resource parameter required")
    end
        ...
        def merge_select_scopes node, resource
          if(node[:select])
            node[:select], for_select_scoping =
              [*node[:select]].partition { |select| resource.has_column?(select) }
...
#          debugger
          node[:select] &&
            node[:select] =
              node[:select].collect {|select| 
                "#{resource.table_name}.#{select}"
              }
        end
      end
    end
