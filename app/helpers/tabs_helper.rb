module TabsHelper
  ...
  @@nav_menu = {
    :primary => {
      :my_submission => {:text => "My Submissions", :controller => 'my_submission', :subnav => [ :my_submissions, :cladename, :edit], :link => '/my_submission' },
      :phenotypes => {:text => "Phenotypes", :controller => 'phenotypes', :subnav => [ ], :link => '/phenotypes' },
      :submissions => { :text => "Review", :controller => 'submissions', :subnav => [ :review_submissions ], :link => '/submissions' },
    ...
    :secondary => {
        :edit => { :title => "Prepare submission", :parent => :my_submission, :path => "show_my_submission_path", :require_id => true },
      :review_submissions => { :title => "Review submissions", :parent => :submissions, :path => "submissions_path" },
      :list_users =>      {:title => 'User List',:parent => :admin, :path => 'admin_path'} ,
      :add_user =>        {:title => 'Add User', :parent => :admin, :path => 'add_user_admin_path'},
      :my_submissions =>  {:title => 'Submissions', :parent => :my_submission, :path => 'my_submission_path'},
        ...
        @@nav_menu[:primary][tab][:subnav].each do |subtab|
          if subtab_path_for subtab
            if controller_name == "my_submission" && controller.action_name.to_s == subtab.to_s
              ...
              byebug
              result_str << "<li class='top_subtab current #{"active" if current_subtab?(subtab)}'>#{link_to "#{@@nav_menu[:secondary][subtab][:title]}",   eval(@@nav_menu[:secondary][subtab][:path])}</li>"
              @sub_tab = subtab
              ...
              result_str << "<li class='top_subtab active#{" current" if current_subtab?(subtab)}'>#{link_to raw("<span>#{@@nav_menu[:secondary][subtab][:title]}</span>"), subtab_path_for(subtab)}</li>"
            end
          end
        end
        result_str << "</ul>"
